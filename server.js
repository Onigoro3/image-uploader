...
// --- ★カテゴリ取得API (修正: typeパラメータで分離) ---
const getCategoryList = (col, level) => apiHandler(async (req, res) => {
    const type = req.query.type; // 'image' or 'csv'
    let query = "", params = [];
    ...
    if (type === 'image') {
        // 画像テーブルから取得
        query = `SELECT DISTINCT ${col} FROM images ${where} ORDER BY ${col}`;
    } else if (type === 'csv') {
        // CSVテーブルから取得
        query = `SELECT DISTINCT ${col} FROM csv_uploads ${where} ORDER BY ${col}`;
    } else {
        // 両方 (デフォルト)
        if (level === 1) {
...
app.get('/api/debug/mismatch', isAuthenticated, apiHandler(async (req, res) => {
    // 紐付いていない商品（CSV側）
    // クエリが重くならないよう、JOINではなくEXISTSを使用し、かつTRIMなど最低限の処理にする
    const prodRows = await pool.query(`
        ...
        AND NOT EXISTS (
            ...
            WHERE i.title LIKE '%' || p.image_filename
        ) 
        LIMIT 5
    `);
    ...
    const imgRows = await pool.query(`
        ...
        WHERE NOT EXISTS (
            ...
            WHERE p.image_filename IS NOT NULL AND p.image_filename <> ''
            AND i.title LIKE '%' || p.image_filename
        )
...
app.post('/api/admin/init-db', isAuthenticated, apiHandler(async (req, res) => { 
    ...
    try {
        ...
        const cols = ['category_1', 'category_2', 'category_3'];
        for (const c of cols) await client.query(`DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='csv_uploads' AND column_name='${c}') THEN ALTER TABLE csv_uploads ADD COLUMN ${c} VARCHAR(100) DEFAULT '未分類'; END IF; END $$;`);
    } finally { client.release(); }
    res.json({ message: 'DB修復完了' }); 
}));
...
app.get('/api/search', isAuthenticated, apiHandler(async (req, res) => {
    ...
    if (q) {
        ...
        keywords.forEach(word => {
            sql += ` AND (i.title ILIKE $${pIdx} OR p.product_name ILIKE $${pIdx} OR p.model_num1 ILIKE $${pIdx} OR p.model_num2 ILIKE $${pIdx} OR p.product_code ILIKE $${pIdx})`;
...
app.delete('/api/cat1/:name', isAuthenticated, apiHandler(async(req, res) => { await pool.query(`DELETE FROM images WHERE category_1=$1`, [req.params.name]); res.json({message:'OK'}); })); // 簡略化
...
app.delete('/api/personal/album/:name', isAuthenticated, apiHandler(async (req, res) => { await pool.query(`DELETE FROM images WHERE category_1='Private' AND category_2='Album' AND category_3='Photo' AND folder_name=$1`, [req.params.name]); res.json({message:"削除完了"}); }));
