...
const { S3Client, DeleteObjectsCommand, CopyObjectCommand, DeleteObjectCommand: S3DeleteObjectCommand } = require('@aws-sdk/client-s3'); // Renamed to avoid conflict
...
app.use(session({
    ...
    secret: process.env.SESSION_SECRET || 'fallback_secret_set_in_env_variable', // ★必ず.envで設定
    resave: false,
...
app.post('/upload', isAuthenticated, upload.array('imageFiles', 100), async (req, res) => {
    ...
    const baseName = folderName.trim(); let highestNumber = 0;
    try {
        const countQuery = `SELECT title FROM images WHERE category_1 = $1 AND category_2 = $2 AND category_3 = $3 AND folder_name = $4 ORDER BY title DESC LIMIT 1`;
        const countResult = await pool.query(countQuery, [category1.trim(), category2.trim(), category3.trim(), baseName]);
        if (countResult.rows.length > 0) { const lastTitle = countResult.rows[0].title; const match = lastTitle.match(/-(\d+)\.[^.]+$/); if (match && match[1]) { highestNumber = parseInt(match[1], 10); console.log(`[Upload] Highest number: ${highestNumber}`); } }
        let nextNumber = highestNumber + 1; const processedFiles = [];
        for (const file of req.files) {
            const tempKey = file.key; const extension = path.extname(tempKey); const newFilename = `${baseName}-${nextNumber}${extension}`; const newKey = newFilename; const newUrl = `${r2PublicUrl}/${encodeURIComponent(newKey)}`;
            console.log(`[Upload] Renaming ${tempKey} to ${newKey}`);
            const copyCommand = new CopyObjectCommand({ Bucket: R2_BUCKET_NAME, CopySource: `${R2_BUCKET_NAME}/${tempKey}`, Key: newKey, ACL: 'public-read' });
            await s3Client.send(copyCommand);
            const deleteCommand = new S3DeleteObjectCommand({ Bucket: R2_BUCKET_NAME, Key: tempKey }); // Use S3DeleteObjectCommand
            await s3Client.send(deleteCommand);
            await pool.query( `INSERT INTO images (title, url, category_1, category_2, category_3, folder_name) VALUES ($1, $2, $3, $4, $5, $6)`, [newFilename, newUrl, category1.trim(), category2.trim(), category3.trim(), folderName.trim()] );
            processedFiles.push(newFilename); nextNumber++; console.log(`[Upload] Saved ${newFilename}`);
        }
        res.json({ message: `「${category1}/${category2}/${category3}/${folderName}」に ${processedFiles.length} 件連番保存` });
    } catch (error) { console.error('[Upload] Error:', error); res.status(500).json({ message: 'ファイル処理エラー' }); }
});
