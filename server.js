...
// ▼▼▼ Vision AI クライアント初期化 (サービスアカウントキーを使う場合 - これだけでOK) ▼▼▼
// (Render Secret File の google-credentials.json を自動で読み込む)
...
async function performDelete(res, conditions, params, itemDescription) {
    try {
        ...
        const deleteResult = await pool.query(`DELETE FROM images WHERE ${conditions}`, params); // Store result
        console.log(`Deleted ${deleteResult.rowCount} DB records for ${itemDescription}`); // Log deleted count
        ...
        for (const image of rows) {
            ...
            try {
                ...
                console.log(`[Analyze GC] Vision AI OK for ${image.title}`);
                 ...
                 console.error(`[Analyze GC] Vision AI Err ${image.title}:`, visionError.message.substring(0, 200));
