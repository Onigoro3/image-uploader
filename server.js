...
app.use(session({
    ...
    secret: process.env.SESSION_SECRET || 'fallback_secret_set_in_env', // Use env var!
    resave: false,
...
app.post('/api/analyze/:folderName', isAuthenticated, async (req, res) => {
    const { folderName } = req.params; console.log(`[Analyze V2] Req: ${folderName}`); let worker;
    ...
    const runOCR = async (buffer, regionName) => { try { const pBuf = await sharp(buffer).grayscale().toBuffer(); const { data: { text } } = await worker.recognize(pBuf); console.log(`[Analyze V2] OCR Ok [${regionName}]`); return text.replace(/"/g, '""').replace(/\n/g, ' ').trim(); } catch (e) { console.error(`[Analyze V2] OCR Err [${regionName}]:`, e.message); return '*** OCR ERR ***'; } };
    try {
        ...
        worker = await createWorker('jpn+eng', 1, { logger: m => console.log(`[Tesseract] ${m.status}: ${m.progress * 100}%`) }); console.log('[Analyze V2] Worker init.');
        let analysisResults = [];
        for (const image of rows) {
            console.log(`[Analyze V2] Processing: ${image.title}`); const result = { filename: image.title, cardName: '', cardText: '', power: '', cost: '', expansion: '' };
            try {
                ...
                analysisResults.push(result); console.log(`[Analyze V2] Processed ${image.title}`);
            } catch (imgError) { console.error(`[Analyze V2] Image Err ${image.title}:`, imgError.message); analysisResults.push({ ...result, cardName: '*** IMG ERR ***' }); }
        }
        ...
        const fileName = `analysis_detailed_${folderName}.csv`; const bom = '\uFEFF'; res.setHeader('Content-Type', 'text/csv; charset=utf-8'); res.setHeader('Content-Disposition', `attachment; filename*=UTF-8''${encodeURIComponent(fileName)}`); res.status(200).send(bom + csvContent); console.log(`[Analyze V2] Sent CSV`);
    } catch (error) { console.error(`[Analyze V2] Error:`, error); if (worker) { try { await worker.terminate(); } catch (e) { console.error('Error terminating worker:', e);} } res.status(500).json({ message: '解析エラー発生' }); }
});
