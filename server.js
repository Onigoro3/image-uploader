...
// ★ sharp と https は解析API内で直接使うので、ここで require しなくてもOK
// const sharp = require('sharp');
// const https = require('https');

// ▼▼▼ Vision AI クライアント初期化 (★Render Secret File パスを指定) ▼▼▼
const visionClient = new vision.ImageAnnotatorClient({
    // RenderがSecret Fileを配置するパスを指定
    keyFilename: '/etc/secrets/google-credentials.json'
});
// ▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲
...
// --- カテゴリ・フォルダ削除API (変更なし) ---
async function performDelete(res, conditions, params, itemDescription) { try { const { rows } = await pool.query(`SELECT title FROM images WHERE ${conditions}`, params); if (rows.length > 0) { const otd = rows.map(r => ({ Key: r.title })); for (let i = 0; i < otd.length; i += 1000) { const c = otd.slice(i, i + 1000); const d = new DeleteObjectsCommand({ Bucket: R2_BUCKET_NAME, Delete: { Objects: c } }); await s3Client.send(d); console.log(`Deleted ${c.length} R2 obj`); } } const dR = await pool.query(`DELETE FROM images WHERE ${conditions}`, params); console.log(`Deleted ${dR.rowCount} DB records`); res.json({ message: `${itemDescription} 削除完了` }); } catch (e) { console.error(`Delete Err:`, e); res.status(500).json({ message: '削除失敗' }); } }
...
app.post('/api/analyze/:folderName', isAuthenticated, async (req, res) => {
    ...
    try {
        ...
        for (const image of rows) {
            ...
            } catch (visionError) { console.error(`[Analyze GC] Vision AI Err ${image.title}:`, visionError.message.substring(0, 200)); analysisResults.push({ filename: image.title, recognizedText: '*** Vision AI エラー ***' }); }
        }
