...
// ★追加: 一時ファイル保存用の設定 (PDF変換用)
const tempUpload = multer({ dest: 'temp_uploads/' }); 
// ※ フォルダ 'temp_uploads' がない場合は自動生成されますが、権限エラーが出る場合は手動作成してください
...
app.post('/api/pdf/convert', isAuthenticated, tempUpload.single('pdfFile'), (req, res) => {
    ...
    // Pythonスクリプトを実行
    // "python" コマンドがサーバーで使える前提です。環境によっては "python3" にしてください。
    const command = `python converter.py "${inputPath}" "${outputPath}"`;
    ...
    exec(command, (error, stdout, stderr) => {
        if (error) {
            ...
            // 失敗しても一時ファイル削除
            fs.unlink(inputPath, ()=>{}); 
            return res.status(500).json({ message: '変換に失敗しました。Python環境を確認してください。' });
        }
    ...
    try {
        ...
        for (const row of records) {
            ...
            await client.query(query, [
                row.product_name, row.model_num1, row.model_num2 || null, row.model_num3 || null, row.model_num4 || null, 
...
app.get('/api/products/files', isAuthenticated, apiHandler(async (req, res) => {
    try { const { rows } = await pool.query("SELECT * FROM csv_uploads WHERE filename <> '__CATEGORY_NODE__' ORDER BY uploaded_at DESC"); res.json(rows); } catch (e) { res.json([]); }
}));
...
app.post('/api/categories/create', isAuthenticated, apiHandler(async (req, res) => {
    ...
    try {
        await client.query('BEGIN');
        const check = await client.query("SELECT id FROM csv_uploads WHERE filename = '__CATEGORY_NODE__' AND category_1=$1 AND category_2=$2 AND category_3=$3", [category1, category2 || '-', category3 || '-']);
        if (check.rows.length === 0) {
            await client.query("INSERT INTO csv_uploads (filename, category_1, category_2, category_3) VALUES ($1, $2, $3, $4)", ['__CATEGORY_NODE__', category1, category2 || '-', category3 || '-']);
        }
        await client.query('COMMIT'); res.json({ message: 'カテゴリを作成しました' });
    } catch (e) { await client.query('ROLLBACK'); throw e; } finally { client.release(); }
}));

app.put('/api/products/files/:id', isAuthenticated, apiHandler(async (req, res) => {
    const { filename, category1, category2, category3 } = req.body;
    await pool.query('UPDATE csv_uploads SET filename = $1, category_1 = $2, category_2 = $3, category_3 = $4 WHERE id = $5', [filename, category1, category2, category3, req.params.id]);
...
const updateCategoryName = async (level, oldName, newName, parent1, parent2) => {
    ...
    try {
        await client.query('BEGIN');
        if (level === 1) {
            await client.query('UPDATE images SET category_1=$1 WHERE category_1=$2', [newName, oldName]);
            await client.query('UPDATE csv_uploads SET category_1=$1 WHERE category_1=$2', [newName, oldName]);
        } else if (level === 2) {
            await client.query('UPDATE images SET category_2=$1 WHERE category_1=$2 AND category_2=$3', [newName, parent1, oldName]);
            await client.query('UPDATE csv_uploads SET category_2=$1 WHERE category_1=$2 AND category_2=$3', [newName, parent1, oldName]);
        } else if (level === 3) {
            await client.query('UPDATE images SET category_3=$1 WHERE category_1=$2 AND category_2=$3 AND category_3=$4', [newName, parent1, parent2, oldName]);
            await client.query('UPDATE csv_uploads SET category_3=$1 WHERE category_1=$2 AND category_2=$3 AND category_3=$4', [newName, parent1, parent2, oldName]);
        }
