...
// 認証
app.post('/api/auth/register', async (req, res) => { const { username, password } = req.body; if (!username || !password || password.length < 8) return res.status(400).json({ message: '長さ不足' }); try { const hash = await bcrypt.hash(password, 10); await pool.query('INSERT INTO users (username, password_hash) VALUES ($1, $2)', [username, hash]); res.status(201).json({ message: '登録成功' }); } catch (e) { res.status(500).json({ message: 'エラー' }); } });
app.post('/api/auth/login', (req, res, next) => { passport.authenticate('local', (err, user, info) => { if (err) return next(err); if (!user) return res.status(401).json({ message: info.message || '失敗' }); req.logIn(user, (err) => { if (err) return next(err); return res.json({ message: '成功', user: user.username }); }); })(req, res, next); });
...
// ★追加: 全商品データ取得API
app.get('/api/products/all', isAuthenticated, apiHandler(async (req, res) => {
    // 負荷対策で最新500件に制限（ページネーションが必要なら別途実装）
    const { rows } = await pool.query('SELECT * FROM product_info ORDER BY updated_at DESC LIMIT 500');
    res.json(rows);
}));

app.get('/api/debug/mismatch', isAuthenticated, apiHandler(async (req, res) => {
    const prodRows = await pool.query(`SELECT product_name, image_filename, product_code, model_num1 FROM product_info p WHERE NOT EXISTS (SELECT 1 FROM images i WHERE (p.image_filename IS NOT NULL AND i.title LIKE '%' || TRIM(p.image_filename)) OR (p.product_code IS NOT NULL AND i.title LIKE '%' || TRIM(p.product_code) || '%')) LIMIT 5`);
...
app.get('/api/search', isAuthenticated, apiHandler(async (req, res) => { const { cat1, cat2, cat3, folder, q, sort, order } = req.query; if (!q && (!cat1 || !cat2 || !cat3 || !folder)) return res.status(400).json({message: 'カテゴリを選択するか、検索ワードを入力してください'}); const s = sort === 'title' ? 'i.title' : 'i.created_at'; const d = order === 'ASC' ? 'ASC' : 'DESC'; let sql = `SELECT i.*, p.product_name, p.model_num1, p.model_num2, p.ec_price, p.mercari_price, p.stock FROM images i LEFT JOIN product_info p ON ((p.image_filename IS NOT NULL AND p.image_filename <> '' AND i.title LIKE '%' || TRIM(p.image_filename)) OR (p.product_code IS NOT NULL AND p.product_code <> '' AND i.title LIKE '%' || TRIM(p.product_code) || '%') OR (p.model_num1 IS NOT NULL AND p.model_num1 <> '' AND i.title LIKE '%' || TRIM(p.model_num1) || '%')) WHERE 1=1`; let params = []; let pIdx = 1; if (cat1) { sql += ` AND i.category_1=$${pIdx++}`; params.push(cat1); } if (cat2) { sql += ` AND i.category_2=$${pIdx++}`; params.push(cat2); } if (cat3) { sql += ` AND i.category_3=$${pIdx++}`; params.push(cat3); } if (folder) { sql += ` AND i.folder_name=$${pIdx++}`; params.push(folder); } if (q) { const keywords = q.replace(/　/g, ' ').trim().split(/\s+/); keywords.forEach(word => { sql += ` AND (i.title ILIKE $${pIdx} OR p.product_name ILIKE $${pIdx} OR p.model_num1 ILIKE $${pIdx} OR p.model_num2 ILIKE $${pIdx} OR p.product_code ILIKE $${pIdx})`; params.push(`%${word}%`); pIdx++; }); } sql += ` ORDER BY ${s} ${d} LIMIT 300`; const { rows } = await pool.query(sql, params); res.json(rows); }));
