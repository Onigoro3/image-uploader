...
// ▼▼▼ Google Cloud Vision AI クライアントを追加 ▼▼▼
const vision = require('@google-cloud/vision');
// ▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲

// ★ Vision AI クライアントを初期化 (APIキーを使う場合)
const visionClient = new vision.ImageAnnotatorClient({
    key: process.env.GOOGLE_VISION_API_KEY // ★ .env または Render 環境変数で設定
});
...
app.use(session({ store: new PgSession({ pool: pool, tableName: 'user_sessions' }), secret: process.env.SESSION_SECRET || 'fallback_secret_set_in_env', resave: false, saveUninitialized: false, cookie: { maxAge: 30 * 24 * 60 * 60 * 1000, secure: process.env.NODE_ENV === 'production', httpOnly: true, sameSite: 'lax' } }));
...
const upload = multer({ storage: multerS3({ s3: s3Client, bucket: R2_BUCKET_NAME, acl: 'public-read', key: (req, file, cb) => { const d = Buffer.from(file.originalname, 'latin1').toString('utf8'); cb(null, d); } }), fileFilter: (req, file, cb) => { if (file.mimetype.startsWith('image/')) { cb(null, true); } else { cb(new Error('画像のみ'), false); } } });
...
// --- カテゴリ・フォルダ削除API (変更なし) ---
async function performDelete(res, conditions, params, itemDescription) { try { const { rows } = await pool.query(`SELECT title FROM images WHERE ${conditions}`, params); if (rows.length > 0) { const otd = rows.map(r => ({ Key: r.title })); for (let i = 0; i < otd.length; i += 1000) { const c = otd.slice(i, i + 1000); const d = new DeleteObjectsCommand({ Bucket: R2_BUCKET_NAME, Delete: { Objects: c } }); await s3Client.send(d); console.log(`Deleted ${c.length} R2 obj`); } } await pool.query(`DELETE FROM images WHERE ${conditions}`, params); res.json({ message: `${itemDescription} 削除完了` }); } catch (e) { console.error(`Delete Err:`, e); res.status(500).json({ message: '削除失敗' }); } }
app.delete('/api/cat1/:name', isAuthenticated, async (req, res) => { await performDelete(res, 'category_1 = $1', [req.params.name], `大「${req.params.name}」`); });
app.delete('/api/cat2/:cat1/:name', isAuthenticated, async (req, res) => { await performDelete(res, 'category_1 = $1 AND category_2 = $2', [req.params.cat1, req.params.name], `中「${req.params.name}」`); });
app.delete('/api/cat3/:cat1/:cat2/:name', isAuthenticated, async (req, res) => { await performDelete(res, 'category_1 = $1 AND category_2 = $2 AND category_3 = $3', [req.params.cat1, req.params.cat2, req.params.name], `小「${req.params.name}」`); });
...
// --- 画像移動API (変更なし) ---
app.put('/api/image/:imageTitle', isAuthenticated, async (req, res) => { const { imageTitle } = req.params; const { category1, category2, category3, folderName } = req.body; if (!category1 || !category2 || !category3 || !folderName) { return res.status(400).json({ message: '移動先指定必須' }); } try { const uQ = `UPDATE images SET category_1 = $1, category_2 = $2, category_3 = $3, folder_name = $4 WHERE title = $5`; const r = await pool.query(uQ, [ category1.trim(), category2.trim(), category3.trim(), folderName.trim(), imageTitle ]); if (r.rowCount === 0) { return res.status(404).json({ message: '画像なし' }); } res.json({ message: `画像移動完了` }); } catch (e) { console.error(`Move Img Err:`, e); res.status(500).json({ message: '移動失敗' }); } });

// ▼▼▼【Google Vision AI 版】フォルダ解析API ▼▼▼
app.post('/api/analyze/:folderName', isAuthenticated, async (req, res) => {
    const { folderName } = req.params;
    console.log(`[Analyze GC] Req: ${folderName}`);
    try {
        ...
        console.log(`[Analyze GC] Found ${rows.length} images`);
        let analysisResults = [];
        for (const image of rows) {
            ...
            try {
                ...
                analysisResults.push({ filename: image.title, recognizedText: fullText.replace(/"/g, '""').replace(/\n/g, ' ') });
                console.log(`[Analyze GC] Vision AI OK for ${image.title}`);
            } catch (visionError) {
                 console.error(`[Analyze GC] Vision AI Err ${image.title}:`, visionError.message.substring(0, 200)); // Log only start of error
                 analysisResults.push({ filename: image.title, recognizedText: '*** Vision AI エラー ***' });
            }
        }
        let csvContent = "ファイル名,認識テキスト(全体)\n";
        analysisResults.forEach(r => { csvContent += `"${r.filename}","${r.recognizedText}"\n`; });
        const fileName = `analysis_vision_${folderName}.csv`; const bom = '\uFEFF';
        ...
        res.status(200).send(bom + csvContent); console.log(`[Analyze GC] Sent CSV`);
